...
TEST_CASE("Case: Member Functions", "[variable]") {
  ...
  auto expr1 = []() { auto f = true + var(true); };

  auto expr2 = [&a, &z]() { auto z = a / 0; };
...
TEST_CASE("Case: computation and nested loop", "[logic]") {
  ...
  for (std::size_t i = 0; i < symbols.size(); ++i) {
    std::string term =
        coeffs[i].as<std::string>() + symbols[i].as<std::string>();
    if (i != symbols.size() - 1) term += " +";
    expression += term;
  }
